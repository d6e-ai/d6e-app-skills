...
### プロバイダ API キー（Gemini、OpenAI、埋め込み）— ローカルでは不要

Plugin の開発やローカルエージェントハーネスから d6e インスタンスに接続する
場合、プロジェクトやエージェント環境に Gemini・OpenAI・その他の埋め込み
プロバイダ用 API キーを設定する必要は**ありません**。インスタンス側が
Gateway キーを保持し、ホストされたチャットエージェントのモデル呼び出しを
ルーティングします。

**`D6E_API_KEY`**（`d6e_…`）は d6e の**ワークスペース API** への認証専用
です — SQL、STF、ワークフロー、SaaS プロキシ、ファイルストレージ、MCP
ツール。LLM プロバイダのキーではありません。
