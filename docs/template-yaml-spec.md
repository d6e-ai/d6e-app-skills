...
Full specification for d6e Plugin manifest files. See the [JSON Schema](../schema/template.schema.json) for machine-readable validation.

## Top-level Fields

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `name` | string | Yes | Plugin name (lowercase, hyphens, `^[a-z0-9][a-z0-9-]*[a-z0-9]$`) |
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `name` | string | Yes | Unique identifier within the plugin. Installed as `{namespace}/{plugin}/{name}` |
...
`SyntaxError: Unexpected token 'export'`. See the d6e-plugin-development
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `name` | string | Yes | Unique identifier. Installed as `{namespace}/{plugin}/{name}` |
...
| `version` | string | No | Defaults to the plugin version. If set, plain semver **without** the `v` prefix (`1.0.0`) — the API rejects `v1.0.0` |
...
- `File`: `{ type: File, id: "UUID" }` — loads a workspace storage file by UUID at execution time. **Caution**: the installer does NOT rewrite this field. The UUID must belong to a file that already exists in the target workspace; it cannot point to files bundled in the plugin (those get fresh UUIDs at install)
